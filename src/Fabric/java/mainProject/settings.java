...
public class settings extends base {
    ...
    @Test(groups = {"Webhooks", "Notifications", "Settings", "Custom Fields"}, priority = 0)
    public void login(){
    ...
    @Test(dependsOnMethods = {"login"}, priority = 1, groups = {"Webhooks","Notifications", "Settings", "Custom Fields"})
    ...
    @Test(dependsOnMethods = {"goto_Settings"}, groups = {"Webhooks","Notifications", "Settings", "Custom Fields"})
    ...
    public int getIndex(List<WebElement> names, String name){
        int length = 0;
        int index = -1;
        System.out.println(names.size());
        while(length < names.size()){
            if(names.get(length).getText().equals(name)){
                System.out.println(names.get(length).getText());
                index = length;
                length = names.size();
            }
            length += 1;
    ...
    @Test(groups = {"Custom Fields"}, dependsOnMethods = {"goto_Settings","validateSettingsMenu"})
    ...
    @Test(groups = {"Custom Fields"}, dependsOnMethods = {"goto_CustomFieldsAndVerify"})
    public void addCustomFieldsAndVerify() throws InterruptedException {
        softAssert.assertTrue(setting.getCustomFieldsAddButton().isDisplayed(), "Add button for custom fields is not available");
        setting.getCustomFieldsAddButton().click();
        driver.switchTo().activeElement();
        softAssert.assertEquals(setting.getCustomFieldsAddTitle().getText(),"Add attribute", "Add Custom Field form has not appropriate Title");
        setting.getCustomFieldsAddAttrDropDown().click();
        Thread.sleep(1500);
        List<WebElement> list = setting.getCustomFieldsAddAttrList();
        list.get(getIndex(list,prop.getProperty("attrType"))).click();
        setting.getCustomFieldsAddName().sendKeys(prop.getProperty("attrName"));
        driver.findElement(By.className("textarea_parent")).click();
        setting.getCustomFieldsAddDescription().sendKeys(prop.getProperty("attrDesc"));
        setting.getCustomFieldsAddSaveButton().click();
        Thread.sleep(2500);
        list = setting.getCustomFieldsAttrList();
        softAssert.assertTrue(check(list,prop.getProperty("attrName")),"Attribute was not saved");
        softAssert.assertAll();
    }
}
